...
export async function buildInviteEmail(opts: {
  inviterName: string;
  orgName: string;
  role: string;
  link: string;
  ...
  return {
    subject: `You have been invited to ${orgName} on RoadmapAI`,
    html: `
<!DOCTYPE html>
<html>
  ...
  <div style="max-width:520px;margin:40px auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
    ...
    <div style="padding:32px 40px;">
      <p style="font-size:15px;color:#0F172A;margin:0 0 16px;line-height:1.6;">
        <strong>${inviterName}</strong> has invited you to join <strong>${orgName}</strong> on RoadmapAI — the AI-powered PMO platform.
      </p>
      <p style="font-size:14px;color:#64748B;margin:0 0 28px;line-height:1.6;">
        You've been invited as <strong style="color:#006D6B;">${role}</strong>. Click below to accept your invitation and get started.
      </p>
...
export async function POST(req: Request) {
  ...
  const existing = await db.invitation.findFirst({
    where: { email: body.email, organisationId: ctx.org.id, status: "PENDING" },
  });
  if (existing)
