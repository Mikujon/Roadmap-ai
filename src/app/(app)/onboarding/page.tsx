      ...
      <div style={{ minHeight: "100vh", background: "#F0F2F5", display: "flex", alignItems: "center", justifyContent: "center", padding: 24, fontFamily: "'DM Sans', sans-serif" }}>
        <div style={{ width: "100%", maxWidth: 560 }}>
          ...
          <div style={{ textAlign: "center", marginBottom: 32 }}>
            ...
            <h1 style={{ fontSize: 26, fontWeight: 800, color: "#18170F", marginBottom: 8, letterSpacing: "-0.5px" }}>Welcome to RoadmapAI</h1>
            <p style={{ fontSize: 14, color: "#5C5A52" }}>Let's get you set up in a few steps</p>
          </div>

          {/* Progress */}
          <div style={{ background: "#fff", border: "1px solid #E5E2D9", borderRadius: 14, padding: "16px 20px", marginBottom: 16, display: "flex", alignItems: "center", gap: 14 }}>
            <div style={{ flex: 1, height: 6, background: "#F4F2EC", borderRadius: 3, overflow: "hidden" }}>
            ...
            <span style={{ fontSize: 12, color: "#9E9C93" }}>{completedSteps}/{steps.length} done</span>
          </div>

          {/* Steps */}
          <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 24 }}>
            {steps.map((step, i) => (
              <div key={step.id} style={{ background: "#fff", border: `1px solid ${step.done ? "#A7F3D0" : i === completedSteps ? "#BFDBFE" : "#E5E2D9"}`, borderRadius: 14, padding: "18px 20px", display: "flex", alignItems: "center", gap: 16, opacity: i > completedSteps ? 0.5 : 1 }}>
                {/* Step indicator */}
                <div style={{ width: 36, height: 36, borderRadius: "50%", background: step.done ? "#ECFDF5" : i === completedSteps ? "#EFF6FF" : "#F8FAFC", border: `2px solid ${step.done ? "#059669" : i === completedSteps ? "#2563EB" : "#E5E2D9"}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, fontSize: 14, fontWeight: 700, color: step.done ? "#059669" : i === completedSteps ? "#2563EB" : "#CCC9BF" }}>
                  {step.done ? "✓" : step.id}
                </div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 14, fontWeight: 700, color: step.done ? "#059669" : "#18170F", marginBottom: 3 }}>{step.title}</div>
                  <div style={{ fontSize: 12, color: "#5C5A52" }}>{step.detail}</div>
                </div>
                {step.action && (
                  <Link href={step.action.href} style={{ padding: "8px 16px", background: i === completedSteps ? "#006D6B" : "#F8FAFC", color: i === completedSteps ? "#fff" : "#5C5A52", border: `1px solid ${i === completedSteps ? "#006D6B" : "#E5E2D9"}`, borderRadius: 8, fontSize: 12, fontWeight: 600, textDecoration: "none", whiteSpace: "nowrap" }}>
                    {step.action.label}
                  </Link>
                )}
              </div>
