...
export function ModalExport({ open, onClose, projectId }: { open: boolean; onClose: () => void; projectId: string }) {
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      ...
      if (format === "pdf") {
        // Open in new tab — browser prints it
      ...
      setErrorMsg((e as Error).message ?? "Export failed");
    } finally {
      setLoading(false);
    }
  }, [projectId, format, onClose]);

  return (
    <Modal id="modal-export" title="Export data" open={open} onClose={onClose}
      footer={
        <>
          <button style={BTN("default")} onClick={onClose} disabled={loading}>Cancel</button>
          <button
            style={BTN("primary")}
      ...
      <div style={FG}>
        ...
        <select style={FI} value={format} onChange={e => setFormat(e.target.value as ExportFormat)}>
          {FORMAT_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
        </select>
      </div>
      <div style={{ ...FG, marginBottom: errorMsg ? 10 : 0 }}>
        <label style={FL}>Includes</label>
        <div style={{ display: "flex", flexDirection: "column", gap: 5, padding: "10px 12px", background: C.surface2, borderRadius: 8, fontSize: 12, color: C.text2 }}>
          {["Project info & KPIs", "EVM metrics (SPI, CPI, EAC, ETC, VAC)", "Features by sprint", "Risk register", "Team assignments"].map(item => (
            <div key={item} style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <span style={{ color: "#006D6B", fontWeight: 700, fontSize: 10 }}>✓</span> {item}
            </div>
          ))}
        </div>
      </div>
      {errorMsg && (
        <div style={{ marginTop: 8, padding: "8px 12px", background: "#FEF2F2", border: "1px solid #FECACA", borderRadius: 7, fontSize: 12, color: "#DC2626" }}>
          {errorMsg}
        </div>
      )}
    </Modal>
