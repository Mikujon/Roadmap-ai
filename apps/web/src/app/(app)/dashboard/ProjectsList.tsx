    ...
    <div style={{ background: "#fff", border: "1px solid #E2E8F0", borderRadius: 14, overflow: "hidden" }}>
      ...
      <div style={{ padding: "14px 20px", borderBottom: "1px solid #F1F5F9", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <span style={{ fontSize: 13, fontWeight: 700, color: "#0F172A" }}>
          Projects <span style={{ fontSize: 11, color: "#94A3B8", fontWeight: 400 }}>{filtered.length} total</span>
        </span>
        <input
          value={search}
          onChange={e => { setSearch(e.target.value); setPage(1); }}
          placeholder="Search projects…"
          style={{ background: "#F8FAFC", border: "1px solid #E2E8F0", borderRadius: 8, padding: "6px 12px", fontSize: 12, outline: "none", width: 200, color: "#0F172A" }}
        />
      </div>

      {/* Table header */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 110px 160px 80px 80px 80px 80px", padding: "8px 20px", background: "#F8FAFC", borderBottom: "1px solid #F1F5F9" }}>
      ...
      {/* Rows */}
      {paginated.length === 0 ? (
        <div style={{ padding: "32px 20px", textAlign: "center", color: "#94A3B8", fontSize: 13 }}>No projects found</div>
      ) : (
        paginated.map(p => {
          ...
          return (
            ...
            >
              <div>
                ...
                <div style={{ fontSize: 11, color: daysLeft < 0 ? "#DC2626" : "#94A3B8" }}>
              ...
              <div>
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  ...
                  <span style={{ fontSize: 11, fontWeight: 600, color: "#0F172A", minWidth: 30 }}>{p.pct}%</span>
                </div>
              </div>
              <div style={{ fontSize: 12, color: "#64748B" }}>{p.done}/{p.allF.length}</div>
              <div style={{ fontSize: 12, color: p.blocked > 0 ? "#DC2626" : "#94A3B8", fontWeight: p.blocked > 0 ? 700 : 400 }}>
        ...
        <div style={{ padding: "12px 20px", borderTop: "1px solid #F1F5F9", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div style={{ display: "flex", gap: 6 }}>
            <button
              ...
              disabled={page === 1}
              style={{ padding: "5px 12px", background: "#F8FAFC", border: "1px solid #E2E8F0", borderRadius: 6, fontSize: 12, cursor: page === 1 ? "default" : "pointer", color: page === 1 ? "#CBD5E1" : "#475569", fontFamily: "inherit" }}
            >← Prev</button>
            {Array.from({ length: totalPages }, (_, i) => i + 1).map(n => (
              <button key={n} onClick={() => setPage(n)}
                style={{ padding: "5px 10px", background: n === page ? "#006D6B" : "#F8FAFC", border: `1px solid ${n === page ? "#006D6B" : "#E2E8F0"}`, borderRadius: 6, fontSize: 12, cursor: "pointer", color: n === page ? "#fff" : "#475569", fontFamily: "inherit", fontWeight: n === page ? 700 : 400 }}
              >{n}</button>
            ))}
            <button
              onClick={() => setPage(p => Math.min(totalPages, p + 1))}
              disabled={page === totalPages}
              style={{ padding: "5px 12px", background: "#F8FAFC", border: "1px solid #E2E8F0", borderRadius: 6, fontSize: 12, cursor: page === totalPages ? "default" : "pointer", color: page === totalPages ? "#CBD5E1" : "#475569", fontFamily: "inherit" }}
            >Next →</button>
          </div>
        </div>
      )}
