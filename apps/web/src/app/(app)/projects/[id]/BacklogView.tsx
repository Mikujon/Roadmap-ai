...
export default function BacklogView({
  allF, updateFeature, canEdit, onOpenNote, allResources = [],
}: {
  allF: Feature[];
  ...
  return (
    <>
      <style>{`
        ...
        .bl-select { background: #F8FAFC; border: 1.5px solid #E2E8F0; border-radius: 8px; padding: 6px 10px; color: #0F172A; font-size: 12px; font-family: inherit; outline: none; cursor: pointer; }
        .bl-select:focus { border-color: #006D6B; }
        .bl-select.active { border-color: #006D6B; background: #ECFDF5; color: #006D6B; font-weight: 600; }
        .bl-search { background: #F8FAFC; border: 1.5px solid #E2E8F0; border-radius: 8px; padding: 7px 12px; color: #0F172A; font-size: 12px; font-family: inherit; outline: none; width: 200px; }
        .bl-search:focus { border-color: #006D6B; background: #fff; }
        .group-pill { padding: 5px 12px; border-radius: 8px; font-size: 12px; font-weight: 600; border: 1.5px solid #E2E8F0; cursor: pointer; font-family: inherit; transition: all 0.15s; background: #fff; color: #64748B; }
        ...
        .bl-table th { background: #FAFBFC; border-bottom: 1px solid #E2E8F0; padding: 8px 14px; text-align: left; font-size: 10px; font-weight: 700; color: #94A3B8; text-transform: uppercase; letter-spacing: 0.07em; white-space: nowrap; }
        .bl-table td { padding: 11px 14px; border-bottom: 1px solid #F1F5F9; vertical-align: middle; }
        .bl-row:hover td { background: #FAFBFC; }
      ...
      <div style={{ display: "flex", gap: 16, marginBottom: 14, padding: "11px 16px", background: "#fff", border: "1px solid #E2E8F0", borderRadius: 12, alignItems: "center" }}>
        {[
          { label: "Total",      value: filtered.length, color: "#0F172A" },
        ...
        {loadingInsights && <span style={{ fontSize: 10, color: "#94A3B8", marginLeft: 8 }}>🛡️ AI analyzing…</span>}
        {hasActiveFilters && (
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {Object.entries(grouped).map(([group, features]) => {
          ...
          return (
            <div key={group} style={{ background: "#fff", border: `1px solid ${gBlocked > 0 ? "#FECACA" : "#E2E8F0"}`, borderRadius: 12, overflow: "hidden" }}>
              <div style={{ padding: "10px 16px", background: gBlocked > 0 ? "#FEF2F2" : "#FAFBFC", borderBottom: "1px solid #F1F5F9", display: "flex", alignItems: "center", gap: 10 }}>
                <span style={{ fontWeight: 700, fontSize: 13, color: "#0F172A" }}>{group}</span>
                <span style={{ fontSize: 11, color: "#94A3B8", background: "#F1F5F9", padding: "1px 8px", borderRadius: 10, fontWeight: 600 }}>{features.length}</span>
                ...
                <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 6 }}>
                  ...
                  <span style={{ fontSize: 11, fontWeight: 700, color: "#0F172A" }}>{gPct}%</span>
                </div>
              </div>
                      ...
                      <tr key={f.id} className={`bl-row${f.status === "BLOCKED" ? " blocked" : ""}`}>
                        ...
                        <td style={{ maxWidth: 280, minWidth: 180 }}>
                          <span
                            ...
                            style={{ fontSize: 13, fontWeight: 500, color: f.status === "DONE" ? "#CBD5E1" : "#0F172A", textDecoration: f.status === "DONE" ? "line-through" : "none" }}
                          >
                            {f.title}
                          </span>
                          {insight && (
                            <div style={{ fontSize: 10, color: "#94A3B8", marginTop: 2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: 260 }}>
                              🛡️ {insight}
                            </div>
                          )}
                        </td>
                        ...
                        <td>
                          <span style={{ fontSize: 11, color: "#64748B", background: "#F1F5F9", padding: "2px 8px", borderRadius: 6, fontWeight: 500 }}>{f.module ?? "—"}</span>
                        </td>
                        ...
                        <td style={{ minWidth: 120 }}>
                          {canEdit ? (
                            ...
                            }} style={{ background: "transparent", border: "none", fontSize: 12, fontFamily: "inherit", color: "#475569", cursor: "pointer", outline: "none", maxWidth: 130 }}>
                              <option value="">— Unassigned —</option>
                              {allResources.map(r => <option key={r.id} value={r.id}>{r.name}</option>)}
                            </select>
                          ) : (
                            <span style={{ fontSize: 12, color: f.assignedTo ? "#0F172A" : "#CBD5E1" }}>
                              {f.assignedTo ? (
                        ...
                        <td>
                          {f.sprintEnd ? (
                            <span
                              title={insight ?? `Sprint: ${f.sprintName}`}
                              style={{ fontSize: 11, color: isOverdue ? "#DC2626" : "#64748B", fontWeight: isOverdue ? 700 : 400, cursor: "help" }}
                            >
                        ...
                        <td>
                          {f.status === "BLOCKED" ? (
                            <button onClick={() => setBlockNote({ id: f.id, text: f.notes ?? "" })}
                              style={{ fontSize: 11, color: f.notes ? "#DC2626" : "#94A3B8", background: f.notes ? "#FEF2F2" : "#F8FAFC", border: `1px solid ${f.notes ? "#FECACA" : "#E2E8F0"}`, borderRadius: 6, padding: "2px 8px", cursor: "pointer", fontFamily: "inherit", fontWeight: 600 }}>
        ...
        {filtered.length === 0 && (
          <div style={{ textAlign: "center", padding: "48px", background: "#fff", border: "1px solid #E2E8F0", borderRadius: 12 }}>
            <div style={{ fontSize: 32, marginBottom: 12 }}>🔍</div>
            <div style={{ fontSize: 14, fontWeight: 600, color: "#0F172A", marginBottom: 6 }}>No tasks found</div>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.3)", zIndex: 100, display: "flex", justifyContent: "flex-end" }} onClick={() => setBlockNote(null)}>
          <div style={{ width: 380, background: "#fff", padding: 24, display: "flex", flexDirection: "column", gap: 16, boxShadow: "-4px 0 24px rgba(0,0,0,0.08)" }} onClick={e => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <button onClick={() => setBlockNote(null)} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 18, color: "#94A3B8" }}>✕</button>
            </div>
            <div style={{ fontSize: 12, color: "#64748B" }}>Document why this task is blocked and what needs to happen to unblock it.</div>
            <textarea value={blockNote.text} onChange={e => setBlockNote(b => b ? { ...b, text: e.target.value } : null)}
              ...
              rows={8} style={{ background: "#F8FAFC", border: "1.5px solid #FECACA", borderRadius: 10, padding: "12px 14px", fontSize: 13, outline: "none", resize: "vertical", lineHeight: 1.6, fontFamily: "inherit", color: "#0F172A" }} />
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <button onClick={() => setBlockNote(null)} style={{ padding: "10px 16px", background: "#F8FAFC", color: "#64748B", border: "1px solid #E2E8F0", borderRadius: 8, fontSize: 13, cursor: "pointer", fontFamily: "inherit" }}>Cancel</button>
            </div>
          </div>
        </div>
      )}
