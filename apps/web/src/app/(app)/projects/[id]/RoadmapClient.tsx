...
function GuardianStrip({ projectId }: { projectId: string }) {
  ...
  return (
    <div style={{
      ...
      borderBottom: `1px solid ${isCritical ? "#FECACA" : "#BBF7D0"}`,
      borderLeft: `3px solid ${isCritical ? "#DC2626" : "#059669"}`,
    }}>
      {/* Strip */}
      <div style={{ padding: "8px 26px", display: "flex", alignItems: "center", gap: 10 }}>
        {/* Pulsing dot */}
        <span className="ai-pulse" style={{ width: 7, height: 7, borderRadius: "50%", background: isCritical ? "#DC2626" : "#059669", flexShrink: 0, display: "inline-block" }} />

        {/* Label */}
        <span style={{ fontSize: 10, fontWeight: 700, color: isCritical ? "#DC2626" : "#059669", textTransform: "uppercase", letterSpacing: "0.07em", flexShrink: 0 }}>
          Guardian AI
        </span>

        {/* Health score pill */}
        {healthScore !== null && (
          <span style={{ fontSize: 10, fontWeight: 800, color: hsColor, background: "#fff", border: `1.5px solid ${hsColor}`, padding: "1px 7px", borderRadius: 99, flexShrink: 0 }}>
            {healthScore}
          </span>
        )}
        ...
        {/* Insight */}
        <span style={{ fontSize: 12, color: loading ? "#94A3B8" : isCritical ? "#991B1B" : "#166534", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
          {loading ? "Analyzing…" : insight ?? "No issues detected."}
        </span>

        {/* Toggle */}
        <button
          onClick={() => setExpanded(e => !e)}
          style={{ fontSize: 11, color: isCritical ? "#DC2626" : "#059669", fontWeight: 600, background: "none", border: "none", cursor: "pointer", fontFamily: "inherit", flexShrink: 0, padding: "2px 0" }}
        >
      ...
      {/* Expanded panel */}
      {expanded && report && (
        <div style={{ padding: "14px 28px 18px", borderTop: `1px solid ${isCritical ? "#FECACA" : "#BBF7D0"}`, background: isCritical ? "#FEF9F9" : "#F7FFF8" }}>
          {/* Score row */}
    ...
    <>
      <style>{`
        *{box-sizing:border-box}
        .tab-btn{padding:6px 16px;border-radius:8px;font-size:12px;font-weight:600;border:none;cursor:pointer;font-family:inherit;transition:all 0.15s}
        ...
        select.light{background:#F8FAFC;border:1.5px solid #E2E8F0;border-radius:8px;padding:8px 12px;color:#0F172A;font-size:13px;font-family:inherit;outline:none;width:100%}
        select.light:focus{border-color:#006D6B}
        .field-input{background:#F8FAFC;border:1.5px solid #E2E8F0;border-radius:8px;padding:9px 12px;color:#0F172A;font-size:13px;font-family:inherit;outline:none}
        .field-input:focus{border-color:#006D6B;background:#fff}
        .status-select{border-radius:8px;font-size:11px;font-weight:700;padding:5px 10px;cursor:pointer;font-family:inherit;outline:none;border-width:1.5px;border-style:solid;}
